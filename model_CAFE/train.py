...
def train(dataset):
    ...
    train_set = FeatureDataset(
        "{}/train_{}_text.npz".format(dataset_dir,dataset),
        "{}/train_{}_img.npz".format(dataset_dir,dataset)
    )
    test_set = FeatureDataset(
        "{}/val_twitter_text.npz".format(dataset_dir),
        "{}/val_twitter_img.npz".format(dataset_dir)
    )
    ...
    best_acc = 0
    sim_best_loss = float('inf')
    det_best_loss = float('inf')
    for epoch in range(num_epoch):
        ...
        # if loss_similarity_test< sim_best_loss:
        #     sim_best_loss= loss_similarity_test
        #     torch.save(similarity_module.state_dict(), 'sim.pt')

        if loss_detection_test < det_best_loss:
            det_best_loss = loss_detection_test
            torch.save(detection_module.state_dict(), '../model/det.pt')
            torch.save(similarity_module.state_dict(), '../model/sim.pt')

        # ---  Output  ---

        print('---  TASK1 Similarity  ---')
        print(
            "EPOCH = %d \n acc_similarity_train = %.4f \n acc_similarity_test = %.4f \n loss_similarity_train = %.4f \n loss_similarity_test = %.4f \n" %
            (epoch + 1, acc_similarity_train, acc_similarity_test, loss_similarity_train, loss_similarity_test)
        )

        print('---  TASK2 Detection  ---')
        print(
            "EPOCH = %d \n acc_detection_train = %.4f \n acc_detection_test = %.4f \n  best_acc = %.4f \n loss_detection_train = %.4f \n loss_detection_test = %.4f \n" %
...
def test_finall(test_loader):
    ...
    similarity_module.load_state_dict(torch.load('../model/sim.pt'))
    detection_module.load_state_dict(torch.load('../model/det.pt'))
    ...
    with torch.no_grad():
        ...
        report = classification_report(detection_pre_label_all, detection_label_all, digits=4)
        print(report)
        print(cm_detection)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='SOTA')
    parser.add_argument('--dataset', type=str, default='wechat')
    args = parser.parse_args()
    train(args.dataset)
    # #跑测试注释掉train
    test_set = FeatureDataset(
        "mydata/test_" + args.dataset + "_text.npz",
        "mydata/test_" + args.dataset + "_img.npz"
    )
    test_loader = DataLoader(
        test_set, batch_size=64, num_workers=1, shuffle=True, drop_last=True
    )

    test_finall(test_loader)
